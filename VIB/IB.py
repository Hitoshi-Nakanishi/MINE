...
from torch.nn.modules import Module, utils
import numpy as np
import pdb

class InformationBottleneck(Module):
    def __init__(self, dim, mask_thresh=0, init_mag=9, init_var=0.01,
                kl_mult=1, divide_w=False, sample_in_training=True,
                 sample_in_testing=False, masking=False, device='cpu'):
    ...
    def forward(self, x):
        def reparameterize(mu, logvar, batch_size, sampling=True):
            ...
            if sampling:
                ...
                eps = torch.FloatTensor(batch_size, std.size(0)).to(self.device).normal_()
